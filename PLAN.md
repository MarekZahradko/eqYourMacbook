...
A tiny, single-purpose menu-bar EQ for the MacBook's **built-in speakers only**.
No windows, no device picker, no per-app routing. Parametric EQ with freely
chosen frequencies and uncapped gain. Near-zero idle cost, no audio glitches
under load, works on battery.

Status: **code complete (written blind on Linux — never compiled), awaiting the
first build session on the Mac.** See "First build session checklist" below.
Owner: Zdeněk. Distribution: personal use (for now); keep a public release possible.
Working title: `eqYourMacbook` (final name TBD; rename is a one-line change in `project.yml`).
...
- **Xcode project generated by XcodeGen** (`project.yml` in repo) — the project
  definition stays plain text, editable from the Linux side; no hand-maintained
  `.pbxproj`. (`brew install xcodegen`, run `xcodegen` after changes.)
- Swift 5.10+, SwiftUI app lifecycle, single target.
...
**M0 — Skeleton.** XcodeGen project, menu-bar icon with empty dropdown, signing,
...
## First build session checklist (2026-06-10 — code written blind, 0 compiles yet)

The whole tree (M0–M5 feature scope) is written and has passed two adversarial
...
- **Option A — user-driven:** code is written into this shared directory; Zdeněk runs
  `xcodegen && xcodebuild` (or Xcode) on the Mac and reports results/listening notes.
